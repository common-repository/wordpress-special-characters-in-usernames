 ...
 */
function wscu_add_setup_link($links, $file)
{
    static $wscu_plugin = null;

    if (is_null($wscu_plugin))
    {
        $wscu_plugin = plugin_basename(__FILE__);
    }

    if ($file == $wscu_plugin)
    {
        $settings_link = '<a href="options-general.php?page=wordpress-special-characters-in-usernames">' . __('Configuration', 'wordpress-special-characters-in-usernames') . '</a>';
        array_unshift($links, $settings_link);
    }

    return $links;
}
add_filter('plugin_action_links', 'wscu_add_setup_link', 10, 2);
